...
innertopmargin=\topskip, frametitle={2019年建模比赛格式变化说明},
...
\item 第三条，论文第三页为摘要专用页（含标题和关键词，但不需要翻译成英文），从此页开始编写页码；页码必须位于每页页脚中部，用阿拉伯数字从“1”开始连续编号。\textbf{\color{red}摘要专用页必须单独一页，且篇幅不能超过一页}。

\item 第四条，从第四页开始是论文正文（\textbf{\color{red}不要目录}，尽量控制在20页以内）；正文之后是论文附录（页数不限）。
...
%\centerline{\includegraphics[width=7cm]{gongzhonghao2}}
...
\begin{figure}[c]
	\centering
	\includegraphics[height=0.2\textheight]{f1.png}
	\caption{多图并排示例}
\end{figure}
\begin{figure}[c]
	\centering
...
\textbf{公式中个别需要加粗}的字母可以用 \verb|$\bm{math symbol}$| 。如 $ \alpha a\bm{\alpha a} $ 。
